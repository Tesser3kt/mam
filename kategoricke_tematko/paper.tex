...
\begin{example}
 ...
 Slovo \uv{představují} zde tajně obsahuje podmínku, že jsou splněny rovnosti,
...
\begin{problem}[0,5 b]
 Rozhodněte, které z následujících obrázků \textbf{mohou} být kategoriemi. Čili,
 u kterých lze najít nějaké rovnosti mezi šipkami (jako na obrázcích v
 ...
 \begin{figure}[H]
  ...
  \begin{subfigure}[b]{.3\textwidth}
   \centering
   \begin{tikzcd}
    & \bullet \ar[dl] \ar[d] \ar[loop,out=45,in=135,looseness=5,shorten
    >=-3pt]\\
    \bullet \ar[loop,out=135,in=225,looseness=4,shorten
    >=-5pt]& \bullet \ar[loop,out=45,in=315,looseness=4,shorten >=-5pt]
   \end{tikzcd}
  \end{subfigure}
  \begin{subfigure}[b]{.3\textwidth}
   \centering
  ...
  \begin{subfigure}[b]{.3\textwidth}
   \centering
   \begin{tikzcd}
    & \bullet \ar[loop,out=45,in=135,looseness=5,shorten >=-3pt] \ar[d]
    \ar[ddl,bend right=45]
    \ar[ddr,bend left=45] &\\
    & \bullet \ar[dl] \ar[dr] \ar[loop,out=45,in=135,looseness=4,shorten
    >=-4pt,shorten <=-3pt,crossing over]&\\
    \bullet \ar[uur] \ar[rr,shift left=.5ex]
    \ar[loop,out=180,in=270,looseness=6,shorten >=-3pt] & &
    \bullet \ar[uul] \ar[ll,shift left=.5ex] 
    \ar[loop,out=0,in=270,looseness=6,shorten >=-3pt]
   \end{tikzcd}
  \end{subfigure}
 \end{figure}
\end{problem}
